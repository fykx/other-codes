def draw(data, pathout):
    ...
    fig, ax = plt.subplots(1,1,figsize=(2.36220472441, 0.8661417), dpi=700, frameon=True)# 0.393700787402
    ...
    ax.set_xlabel('Elevation (m)', fontdict={'color': 'black', 'weight': 'normal', 'size':8})
    ax.set_ylabel('TCC mean (%)', fontdict={'color': 'black', 'weight': 'normal', 'size':8})

    ax.set_xticks(range(0, 3000, 500))
    ax.set_xticklabels(range(0, 3000, 500), fontsize=8, rotation=0)
    ax.set_yticks(range(0, 60, 15))
    ax.set_yticklabels(range(0, 60, 15), fontsize=8, rotation=0)   
